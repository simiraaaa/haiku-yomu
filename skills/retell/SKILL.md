...
Write the Step 1 intent (premise and answer key) **for every document up front**, and do the Step 3 comparison per document after all results are in (a single combined report is fine). **Running the same document multiple times (3–5) works the same way — launch the runs in parallel** (a fluctuation countermeasure for important documents; write each run's output to its own file. How to cross-check the reports across runs is in Step 3). Note: even when you run all files in parallel, each inspection sees only its own document — **all documents passing does not mean the documents are consistent with each other** (as stated under "Out of scope"; cross-file consistency belongs to cross-cutting reviews).
...
- With Haiku and no tools, a run normally returns in under a minute. Synchronous execution is fine for one document (for multiple documents, launch in parallel as above).
...
**Judge question 3 item by item**: self-reports often include detail demands the assumed reader does not need (e.g. "spell out internal spec values") — measured in the author's environment, 50–70% of them are noise. Do not take them at face value; judge each one. When you have run the same document several times (3–5) in parallel (launched the same way as the multi-document parallel runs), cross-check the reports across runs before judging item by item — **prioritize reports that recur in multiple runs, and treat a report that appears in only one run as a weak signal** (in the author's measurement of 5 runs on a single document, 7 of 9 report types were single-run, and the recurring ones matched real weaknesses; treat the ratio as a rough guide from that one document). Whether fixes have landed can also be gauged by the retelling (questions 1–2) — question 3 keeps producing minor reports even as the document improves. That gauge is only an input, though: whether the re-verification loop ends is decided by the user's explicit instruction (Step 5).
...
When the user gives instructions, apply the fixes, re-run Step 2, and repeat Steps 3–4. Haiku is stateless, and having it read as a completely fresh reader each time is the point — a reader retaining memory of the previous read can compensate from context and understand even an unfixed document, which is no re-verification at all. Therefore never resume the session (`claude -p --resume` to carry over the previous reading session).
...
| Retelling quality fluctuates between runs on the same document | Probabilistic inspection (by design) | Do not read one pass as a guarantee. For important documents, run the same document 3–5 times in parallel and prioritize reports that recur across runs (a single-run report is a weak signal; judging is in Step 3). Nor does one passing run settle the verdict — in the author's 5-run measurement of a single document, 1 run largely failed to retell the post-reading actions |
